...
export default function BitDetail() {
          ...
          .forEach(event =>
            event.topCut?.forEach(player =>
              player.combos?.forEach(({ blade, ratchet, bit }) => {
                if (bit.trim().toLowerCase() === name?.trim().toLowerCase()) {
                  const key = `${blade.trim()}::${ratchet.trim()}`
                  if (countMap.has(key)) {
  ...
  const readable = name?.replace(/-/g, " ") ?? "Bit"
  ...
  return (
    <>
      <Helmet>
        ...
        <meta property="og:url" content={`https://www.metabeys.com/bit/${name}`} />
      </Helmet>
      ...
      <motion.div className="p-6 max-w-4xl mx-auto" initial={{ opacity: 0 }} animate={{ opacity: 1 }}>
        ...
        <p className="text-neutral-content mb-6">Top Combos for this bit:</p>
        ...
        <div className="grid gap-4">
          {paginatedCombos.map((combo, i) => {
            const slug = encodeURIComponent(
              `${combo.blade.toLowerCase()}-${combo.ratchet.toLowerCase()}-${name?.toLowerCase().trim()}`
            )
          ...
          {!combos.length && <p className="text-error text-sm">No combos found for this bit.</p>}
        </div>
